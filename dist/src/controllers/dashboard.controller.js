"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.getServicesByCategory = exports.getEarningsOverTime = exports.getDashboardKpis = void 0;
const database_1 = require("../config/database");
const sequelize_1 = require("sequelize");
const Professional_1 = require("../models/Professional");
const getDashboardKpis = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    const authReq = req;
    const userId = (_a = authReq.user) === null || _a === void 0 ? void 0 : _a.id;
    if (!userId)
        return res.status(401).json({ error: "Usuário não autenticado" });
    const professional = yield Professional_1.ProfessionalModel.findOne({ where: { user_id: userId } });
    if (!professional)
        return res.status(404).json({ error: "Profissional não encontrado para este usuário" });
    const professionalId = professional.id;
    try {
        const sql = `
      SELECT
        COUNT(*) AS totalServices,
        COALESCE(SUM(COALESCE(a.final_price, s.price)), 0) AS totalEarnings,
        AVG(a.rating) AS avgRating
      FROM appointment a
      JOIN service s ON s.id = a.service_id
      WHERE a.professional_id = :id
        AND a.status = 'completed'
    `;
        const [results] = yield database_1.sequelize.query(sql, {
            replacements: { id: professionalId },
            type: sequelize_1.QueryTypes.SELECT,
        });
        return res.json({
            totalServices: Number(results.totalServices || 0),
            totalEarnings: parseFloat(String(results.totalEarnings || 0)),
            avgRating: results.avgRating !== null ? parseFloat(String(results.avgRating)) : undefined,
        });
    }
    catch (error) {
        console.error("Error in getDashboardKpis:", error);
        return res.status(500).json({ error: "Erro interno" });
    }
});
exports.getDashboardKpis = getDashboardKpis;
const getEarningsOverTime = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    const authReq = req;
    const userId = (_a = authReq.user) === null || _a === void 0 ? void 0 : _a.id;
    if (!userId)
        return res.status(401).json({ error: "Usuário não autenticado" });
    const professional = yield Professional_1.ProfessionalModel.findOne({ where: { user_id: userId } });
    if (!professional)
        return res.status(404).json({ error: "Profissional não encontrado para este usuário" });
    const professionalId = professional.id;
    try {
        const { from, to } = req.query;
        const now = new Date();
        const defaultFrom = new Date(now);
        defaultFrom.setMonth(defaultFrom.getMonth() - 11);
        defaultFrom.setDate(1);
        const parseOrDefault = (d, fallback) => {
            if (!d)
                return fallback;
            const parsed = new Date(d);
            if (isNaN(parsed.getTime()))
                return fallback;
            return parsed;
        };
        const fromDate = parseOrDefault(from, defaultFrom);
        const toDate = parseOrDefault(to, now);
        const queryProfessionalId = req.query.professionalId;
        const idToUse = queryProfessionalId ? Number(queryProfessionalId) : professionalId;
        if (!idToUse || isNaN(idToUse))
            return res.status(400).json({ error: "professionalId inválido" });
        const fromReplacement = fromDate.toISOString().slice(0, 19).replace("T", " ");
        const toReplacement = toDate.toISOString().slice(0, 19).replace("T", " ");
        const dialect = (database_1.sequelize.getDialect && database_1.sequelize.getDialect()) || process.env.SEQUELIZE_DIALECT || "mysql";
        let monthExpr;
        if (dialect === "postgres" || dialect === "postgresql") {
            monthExpr = "to_char(a.completed_at, 'MM-YYYY')";
        }
        else {
            monthExpr = "DATE_FORMAT(a.completed_at, '%m-%Y')";
        }
        const sql = `
      SELECT ${monthExpr} AS month,
             SUM(COALESCE(a.final_price, s.price)) AS total
      FROM appointment a
      JOIN service s ON s.id = a.service_id
      WHERE a.professional_id = :id
        AND a.status = 'completed'
        AND a.completed_at IS NOT NULL
        AND a.completed_at BETWEEN :from AND :to
      GROUP BY DATE_TRUNC_CANDIDATE
      ORDER BY DATE_TRUNC_CANDIDATE
    `;
        let finalSql = sql;
        if (dialect === "postgres" || dialect === "postgresql") {
            finalSql = finalSql.replace(/DATE_TRUNC_CANDIDATE/g, "EXTRACT(YEAR FROM a.completed_at), EXTRACT(MONTH FROM a.completed_at)");
        }
        else {
            finalSql = finalSql.replace(/DATE_TRUNC_CANDIDATE/g, "YEAR(a.completed_at), MONTH(a.completed_at)");
        }
        const results = yield database_1.sequelize.query(finalSql, {
            replacements: {
                id: idToUse,
                from: fromReplacement,
                to: toReplacement,
            },
            type: sequelize_1.QueryTypes.SELECT,
        });
        const mapped = results.map(r => ({ month: r.month, total: parseFloat(String(r.total || 0)) }));
        return res.json(mapped);
    }
    catch (error) {
        console.error("Error in getEarningsOverTime:", (error === null || error === void 0 ? void 0 : error.stack) || error);
        return res.status(500).json({ error: "Erro interno" });
    }
});
exports.getEarningsOverTime = getEarningsOverTime;
const getServicesByCategory = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    const authReq = req;
    const userId = (_a = authReq.user) === null || _a === void 0 ? void 0 : _a.id;
    if (!userId)
        return res.status(401).json({ error: "Usuário não autenticado" });
    const professional = yield Professional_1.ProfessionalModel.findOne({ where: { user_id: userId } });
    if (!professional)
        return res.status(404).json({ error: "Profissional não encontrado para este usuário" });
    const professionalId = professional.id;
    try {
        const { from, to } = req.query;
        const now = new Date();
        const defaultFrom = new Date(now);
        defaultFrom.setMonth(defaultFrom.getMonth() - 11);
        defaultFrom.setDate(1);
        const parseOrDefault = (d, fallback) => {
            if (!d)
                return fallback;
            const parsed = new Date(d);
            if (isNaN(parsed.getTime()))
                return fallback;
            return parsed;
        };
        const fromDate = parseOrDefault(from, defaultFrom);
        const toDate = parseOrDefault(to, now);
        const sql = `
      SELECT c.title AS category, COUNT(*) AS count
      FROM appointment a
      JOIN service s ON s.id = a.service_id
      JOIN subcategory sc ON sc.id = s.subcategory_id
      JOIN category c ON c.id = sc.category_id
      WHERE a.professional_id = :id
        AND a.status = 'completed'
        AND a.completed_at BETWEEN :from AND :to
      GROUP BY c.id, c.title
      ORDER BY count DESC
    `;
        const results = yield database_1.sequelize.query(sql, {
            replacements: {
                id: professionalId,
                from: fromDate.toISOString().slice(0, 19).replace("T", " "),
                to: toDate.toISOString().slice(0, 19).replace("T", " "),
            },
            type: sequelize_1.QueryTypes.SELECT,
        });
        const mapped = results.map(r => ({ category: r.category, count: Number(r.count) }));
        return res.json(mapped);
    }
    catch (error) {
        console.error("Error in getServicesByCategory:", error);
        return res.status(500).json({ error: "Erro interno" });
    }
});
exports.getServicesByCategory = getServicesByCategory;
